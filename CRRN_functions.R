...
# times <- seq(from=0, to=50*365, by=30)
options(scipen=999)
...
Probs <- function(M_it, j) {
  ...
  # Update p.it with appropriate probabilities
  p.it[, M_it == "DX"]  <- rbind(1 - p.DXEDC[pos.DX]  - p.DXEDO[pos.DX]  - p.DXHC[pos.DX] - p.DXHO[pos.DX] - p.DXMO[pos.DX], p.DXEDC[pos.DX], p.DXEDO[pos.DX], p.DXHC[pos.DX], p.DXHO[pos.DX], p.DXMO[pos.DX])# transition probabilities from DX
  p.it[, M_it == "EDC"] <- rbind(1 - p.EDCHC[pos.EDC] - p.EDCHO[pos.EDC] - p.EDCMO[pos.EDC], 0, 0, p.EDCHC[pos.EDC], p.EDCHO[pos.EDC], p.EDCMO[pos.EDC])  # transition probabilities from EDC
  p.it[, M_it == "EDO"] <- rbind(1 - p.EDOHC[pos.EDO] - p.EDOHO[pos.EDO] - p.EDOMO[pos.EDO], 0, 0, p.EDOHC[pos.EDO], p.EDOHO[pos.EDO], p.EDOMO[pos.EDO])  # transition probabilities from EDO
  p.it[, M_it == "HC"]  <- rbind(p.HCDX[pos.HC], 0, 0, 1 - p.HCDX[pos.HC] - p.HCMO[pos.HC], 0, p.HCMO[pos.HC])                                            # transition probabilities from HC
  p.it[, M_it == "HO"]  <- rbind(p.HODX[pos.HO], 0, 0, 0, 1 - p.HODX[pos.HO] - p.HOMO[pos.HO], p.HOMO[pos.HO])                                            # transition probabilities from HO
  p.it[, M_it == "MO"]  <-     c(0, 0, 0, 0, 0, 1)                                                                                                        # transition probabilities from MO
  
  #ifelse(sum(p.it)==1, return(p.it), print ("Probabilites do not sum to 1")) 
...
############################## Import ADG GEE ###################################
#   GEEADG <- t(read_excel("20190104_CRRN_Models_EDOHO_noADG.xlsx", sheet="GEE_ADG"))
#   GEEADG 
#   
# ##### ADG function for simulation #####
#   ADGcycle  <- function(betas , X){
#     ADG_res <- exp(X%*%betas)
#     return(ADG_res)
#   }
